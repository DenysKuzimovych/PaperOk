      ...
      <div className="grid gap-5 md:grid-cols-3">
        {categories.map((cat, index) => (
          <Link
            key={cat.href}
            href={cat.href}
            className="group relative flex min-h-[280px] flex-col overflow-hidden rounded-[1.5rem] border border-paper-border/70 bg-paper-white transition-transform duration-300 hover:-translate-y-1"
            style={{ boxShadow: "var(--paper-shadow)" }}
          >
            <div
              className={`relative h-36 bg-gradient-to-br ${cat.tone}`}
            >
